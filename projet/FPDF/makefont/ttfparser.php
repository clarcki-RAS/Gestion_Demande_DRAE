...
class TTFParser
{
	...
	function ParseOffsetTable()
	{
		...
		$this->Skip(3*2); // rechercherRange, entrySelector, rangeShift
		$this->tables = array();
...
/*		$chars = array_keys($this->chars);
	...
	function BuildCmap()
	{
		...
		$rechercherRange = (1<<$entrySelector)*2;
		$rangeShift = 2*$segCount - $rechercherRange;
		$cmap = pack('nnnn', 2*$segCount, $rechercherRange, $entrySelector, $rangeShift);
...
//		$this->tables['head']['data'] = substr_replace($this->tables['head']['data'], "\x00\x00\x00\x00", 8, 4);
		...
		$rechercherRange = 16*(1<<$entrySelector);
		$rangeShift = 16*$numTables - $rechercherRange;
		$offsetTable = pack('nnnnnn', 1, 0, $numTables, $rechercherRange, $entrySelector, $rangeShift);
		foreach($tags as $tag)
		{
