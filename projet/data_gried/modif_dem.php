...
$query = ("SELECT * FROM demander WHERE ID_PAYSANT ='$id_paysant' " );
...
{  
    ...
    if(isset($id_responsable)  && isset($id_paysant)
    && !empty($id_responsable) && !empty($id_paysant) && !   empty($date_envoie) && !empty($date_reponse) && !empty($filiere) && !empty($appui) &&
    !empty($materiel) ) 
	{
		

		$query = "UPDATE demander SET ID_RESPONSABLE=?,ID_PAYSANT=?,DATE_ENVOIE=? ,DATE_REP=?,FILIERE=?,TYPE_APPUI=?,MATERIEL=?  WHERE ID_PAYSANT=$ID_PAYSANT";
        $pdostmt=$bdd->prepare($query);
        $reponse = $pdostmt->execute(array($id_responsable,$ID_PAYSANT,$date_envoie,$date_reponse,$filiere,$appui,$materiel));
		...
		if($reponse)
		{
			...
			<?php
            header("location:data_demande.php");
		}
					...
					<div class="row">
						<div class="col-md-12">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-md-6 col-lg-4">
											...
											<form name="insertion1" action="modif_dem.php" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" id="date_envoie"  name="date_envoie"value="<?=$row['DATE_ENVOIE']?>">
                            </div>
                            <div class="form-group">
                                <label for="contact">date de reponse </label>
                                <input type="date" class="form-control" id="date_reponse" name="date_reponse"value="<?=$row['DATE_REP']?>">
                            </div>
                            <div class="form-group">
                                <label for="contact">filiere d'activite </label>
                                <input type="text" class="form-control" id="filiere" placeholder="branche d'activite" name="filiere"value="<?=$row['FILIERE']?>">
                            </div>
                            <div class="form-group">
                                <label for="contact">type d'appuis demamder </label>
                                <input type="text" class="form-control" id="appui" placeholder="technique / materiel" name="appui"value="<?=$row['TYPE_APPUI']?>">
                            </div>
                            <div class="form-group">
                                <label for="contact">materiel demander </label>
                                <input type="text" class="form-control" id="materiel" placeholder="Materiel demander par le paysant " name="materiel" value="<?=$row['MATERIEL']?>">
                            </div>
