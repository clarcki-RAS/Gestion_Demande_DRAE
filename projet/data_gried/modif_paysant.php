...
$query = ("SELECT * FROM paysant WHERE ID_PAYSANT ='$id_paysant' " );
...
{  
    ...
    if( !empty($nom_ass) && !empty($date_naiss) && !   empty($cin) && !empty($numero) && !empty($region) && !empty($district) &&
    !empty($commune) && !empty($village) && !empty($fokontany) && !empty($sexe) && !empty($nom_paysant) ) 
	{
		

		$query = "UPDATE paysant SET NOM_ASSOC=?,NOM_EAF=?,DATE_NAISS=? ,SEXE=?,CIN_PAYSANT=?,REGION=?,COMMUNE=? ,DISTRICT=?,VILLAGE=?,FOKONTANY=?,NUM_TEL_PAY=? WHERE ID_PAYSANT=$ID_PAYSANT";
        $pdostmt=$bdd->prepare($query);
        $reponse = $pdostmt->execute(array($nom_ass,$nom_paysant,$date_naiss,$sexe,$cin,$region,$commune,$district,
					...
					<div class="row">
						<div class="col-md-12">
								...
								<div class="card-body">
									<div class="row">
										<div class="col-md-6 col-lg-4">
											...
											<form name="insertion1" action="modif_paysant.php" method="POST">
                                            <div class="form-group">
                                <label for="id_pay">id_paysan </label>
                                <input type="number" class="form-control" id="id_pay" placeholder="identifiant du paysant" name="id_pay"
                                value="<?=$row['ID_PAYSANT']?>" readonly="readonly">
                            </div>
                            <div class="form-group">
                                <label for="nom_paysant">Nom et Prenom</label>
                                <input type="text" class="form-control" id="nom_paysant" placeholder="nom du paysant" name="nom_paysant"
                                value="<?=$row['NOM_EAF']?>">
                            </div>
                            <div class="form-group">
                                <label for="nom_ass">Nom de l'association </label>
                                <input type="text" class="form-control" id="nom_ass" placeholder="nom association"  name="nom_ass"
                                value="<?=$row['NOM_ASSOC']?>">
                            </div>
                            <div class="form-group">
                                <label for="date_naiss">Date De Naissance </label>
                                <input type="date" class="form-control" id="date_naiss" placeholder="branche d'activite" name="date_naiss"
                                value="<?=$row['DATE_NAISS']?>">
                            </div>
                            <div class="form-group">
                                <label for="sexe">Sexe</label>
                                <input type="text" class="form-control" id="sexe" placeholder="homme /femme" name="sexe"
                                value="<?=$row['SEXE']?>">
                            </div>
                            
                            <div class="form-group">
                                <label for="cin">Numero CIN </label>
                                <input type="number" class="form-control" id="cin" placeholder="xxxxxxxxxxxxxx" name="cin"
                                value="<?=$row['CIN_PAYSANT']?>">
                            </div>
                            
                            <div class="form-group">
                                <label for="region">Region</label>
                                <input type="text" class="form-control" id="region" placeholder="region cible"  name="region"
                                value="<?=$row['REGION']?>">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="commune" placeholder="commune urbaine"  name="commune"
                                value="<?=$row['COMMUNE']?>">
                            </div>
                            <div class="form-group">
                                <label for="village">Tanana </label>
                                <input type="text" class="form-control" id="village" placeholder="tanana"  name="village"
                                value="<?=$row['VILLAGE']?>">
                            </div>
                            <div class="form-group">
                                <label for="fokontany">fokontany </label>
                                <input type="text" class="form-control" id="fokontay" placeholder="fokontany"  name="fokontany"
                                value="<?=$row['FOKONTANY']?>">
                            </div>
                            <div class="form-group">
                                <label for="num">Telephone </label>
                                <input type="tel" class="form-control" id="num" placeholder="03x xx xxx xx" name="num"
                                value="<?=$row['NUM_TEL_PAY']?>">
                            </div>
